...
class AirtableClient:
    ...
    def __init__(self, base_id: str, table_name: str, api_key: str | None = None):
        ...
        logger.info(
            f"Airtable client initialized for base: {base_id}, table: {table_name}"
        )
    ...
    def get_leads(self) -> list[Lead]:
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                logger.error(
                    "Base or table not found - check your base ID and table name"
                )
            raise
    ...
    def add_lead(self, lead: Lead) -> dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                self.endpoint, headers=self.headers, json=data, timeout=30
            )
            response.raise_for_status()
    ...
    def delete_lead(self, record_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = requests.delete(
                f"{self.endpoint}/{record_id}", headers=self.headers, timeout=30
            )
            response.raise_for_status()
