...
class OutreachAgent:
    ...
    def _generate_personalized_message(self, lead: dict) -> str | None:
        ...
        try:
            lead_dict = {"name": lead["Name"], "company": lead["Company"]}
            message = self.anthropic_client.generate_outreach_message(
                lead_dict, self.config.client_info, self.config.offer
            )
            processing_time = time.time() - start_time
            self.performance_metrics["message_generation_time"] += processing_time
            logger.info(
                f"Generated personalized message for {lead['Name']} at {lead['Company']} in {processing_time:.2f}s"
            )
            return message
        except Exception as e:
            logger.error(
                f"Failed to generate message for {lead.get('Name', 'Unknown')}: {e}"
            )
            return None
    ...
    def _send_outreach_email(self, lead: dict, message: str) -> bool:
        ...
        try:
            ...
            self.email_sender.send_email(
                lead["Email"], subject, message, from_email=self.config.from_email
            )
            processing_time = time.time() - start_time
            self.performance_metrics["email_sending_time"] += processing_time
            logger.info(
                f"Successfully sent email to {lead['Name']} <{lead['Email']}> in {processing_time:.2f}s"
            )
            return True
        except Exception as e:
    ...
    def _create_followup_task(self, lead: dict, message: str) -> bool:
        ...
        try:
            ...
            self.clickup_client.create_task(
                self.config.clickup_list_id, task_name, description=message
            )
            processing_time = time.time() - start_time
            self.performance_metrics["task_creation_time"] += processing_time
            logger.info(
                f"Created ClickUp task for {lead['Name']} at {lead['Company']} in {processing_time:.2f}s"
            )
            return True
        except Exception as e:
            logger.error(
                f"Failed to create ClickUp task for {lead.get('Name', 'Unknown')}: {e}"
            )
            return False
    ...
    def get_performance_metrics(
        ...
        return {
            ...
            "message_generation_time": self.performance_metrics[
            ...
            "average_message_time": self.performance_metrics["message_generation_time"]
            / max(campaign_stats.get("messages_generated", 1), 1),
            "average_email_time": self.performance_metrics["email_sending_time"]
            / max(campaign_stats.get("emails_sent", 1), 1),
            "average_task_time": self.performance_metrics["task_creation_time"]
            / max(campaign_stats.get("tasks_created", 1), 1),
        }
